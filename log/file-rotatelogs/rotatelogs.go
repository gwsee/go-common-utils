...
func (rl *RotateLogs) Write(p []byte) (n int, err error) {
	...
	out, err := rl.getWriter_nolock(false, false, int64(len(p)))
	if err != nil {
...
func (rl *RotateLogs) getWriter_nolock(bailOnRotateFail, useGenerationalNames bool, logSize int64) (io.Writer, error) {
	sizeRotate := false
	generation := rl.generation
	sizeGeneration := rl.sizeGeneration
	previousFn := rl.curFn
	...
	if baseFn != rl.curBaseFn {
		...
		if sizeGeneration > 0 {
			filename = fmt.Sprintf("%s-%d", baseFn, sizeGeneration)
		}
		// the same basename. inspect the file size
		fs, err := os.Stat(filename)
		if err != nil {
			return rl.outFh, err
		}
		if rl.maxSize-fs.Size() < logSize {
			sizeRotate = true
			// rotate file
		}

		if !useGenerationalNames && !sizeRotate {
			// nothing to do
			return rl.outFh, nil
		}
		...
		if sizeRotate {
			for {
				sizeGeneration++
				name := fmt.Sprintf("%s-%d", baseFn, sizeGeneration)
				if _, err := os.Stat(name); err != nil {
					filename = name
					break
				}
	...
	// output panic info into log file
	redirectStderr(fh)
	...
	rl.sizeGeneration = sizeGeneration
...
func (rl *RotateLogs) Rotate() error {
	rl.mutex.Lock()
	defer rl.mutex.Unlock()
	if _, err := rl.getWriter_nolock(true, true, 0); err != nil {
		return err
	}
	return nil
}
