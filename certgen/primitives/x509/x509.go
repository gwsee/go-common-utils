...
// To verify SM2 signatures, digest should be padded by the follow mathod
...
func checkSignature(algo SignatureAlgorithm, signed, signature []byte, publicKey crypto.PublicKey) (err error) {
	...
	if algo == SM3WithSM2 {
		pk, ok := publicKey.(*sm2.PublicKey)
		if ok {
			if !pk.Verify(signature, SignHashSM3(common.BigToBytes(pk.X, 10), common.BigToBytes(pk.Y, 10), signed)) { //pk.VerifySignature(signature, SignHashSM3(pk.X, pk.Y, signed))
...
func parsePublicKey(algo PublicKeyAlgorithm, keyData *publicKeyInfo) (interface{}, error) {
	...
	case ECDSA:
		...
		//参见keygen的MarshalSm2PublicKey函数
		//进入if说明是SM2公钥
...
func CreateCertificate(rand io.Reader, template, parent *Certificate, pub, priv interface{}, useGruomi bool) (cert []byte, err error) {
	...
	if useGruomi && !publicKeyAlgorithm.Algorithm.Equal(oidPublicKeySM2) {
		return nil, errors.New("guomi ca but non-guomi publick key")
	}
	...
	var digest []byte
	if useGruomi {
		p := key.Public().(*sm2.PublicKey)
		digest = SignHashSM3(common.BigToBytes(p.X, 10), common.BigToBytes(p.Y, 10), tbsCertContents)
	} else {
		h := hashFunc.New()
...
func CreateCertificateRequest(rand io.Reader, template *CertificateRequest, priv interface{}) (csr []byte, err error) {
	...
	hashFunc, sigAlgo, err = signingParamsForPublicKey(key.Public().(*sm2.PublicKey), template.SignatureAlgorithm)
	if err != nil {
		return nil, err
	}
