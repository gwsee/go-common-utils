...
func GetPubKeyFromPri(pri *PrivateKey) []byte {
	return bytes.Join([][]byte{{4}, pri.X.Bytes(), pri.Y.Bytes()}, nil)
}

func (key *PublicKey) GetRaw() (string, error) {
	var hexX []byte
	bigy := common.BytesToBig(key.Y.Bytes())
	if new(big.Int).Mod(bigy, big.NewInt(2)).Int64() == 1 {
		hexX = append([]byte{3}, key.X.Bytes()...)
	} else {
		hexX = append([]byte{2}, key.X.Bytes()...)
	}
...
func ParsePublicKeyByEncode(der []byte) (*PublicKey, error) {
	...
	key.X = new(big.Int).SetBytes(raw[:intLength])
	//key.Y = make([]byte, intLength)
	key.Y = new(big.Int).SetBytes(raw[intLength:])
...
func GetPriKeyFromHex(pri []byte) (*PrivateKey, error) {
	priv := new(PrivateKey)
	c := P256Sm2()
	privX, privY := c.ScalarBaseMult(pri)
	// k, err := randFieldElement(c, rand.Reader)
	// if err != nil {
	// 	return nil, err
	// }
	priv.PublicKey.Curve = c
	priv.D = big.NewInt(0).SetBytes(pri)
	priv.PublicKey.X, priv.PublicKey.Y = privX, privY
	return priv, nil
}
...
func ParseSMPrivateKey(der []byte) (interface{}, error) {
	...
	if err == nil {
		...
		x, y := elliptic.Unmarshal(P256Sm2(), privGmssl.PublicKey.Bytes)
		return &PrivateKey{
			PublicKey: PublicKey{
				Curve: P256Sm2(),
				X:     x,
	...
	if err == nil {
		...
		x, y := elliptic.Unmarshal(P256Sm2(), privCFCA.PublicKey.Bytes)
		return &PrivateKey{
			PublicKey: PublicKey{
				Curve: P256Sm2(),
				X:     x,
